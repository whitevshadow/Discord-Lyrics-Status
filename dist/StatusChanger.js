...
function pickEmojiForLyric(text) {
  ...
  for (let i = 0; i < emojiByLyricList.length; i++) {
    for (let j = 0; j < emojiByLyricList[i].keywords.length; j++) {
      if (lower.includes(emojiByLyricList[i].keywords[j])) return emojiByLyricList[i].emoji;
    }
  }
  return "🎶";
}
...
class StatusChanger {
  ...
  changeStatus() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.time < (songProgress + offset)) {
        ...
        if (this.sentLines.some(function(sentLine) { return sentLine.time === line.time; })) break;
  ...
  getStatusString(line) {
    return (Settings_1.Settings.view.timestamp ? "[" + this.formatSeconds(+(line.time / 1000).toFixed(0)) + "] " : "")
      + (Settings_1.Settings.view.label ? "Song lyrics - " : "")
      + line.text.replace("♪", "🎶")
      .slice(0, 128);
  }
