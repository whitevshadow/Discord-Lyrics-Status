...
const emojiByLyric: { [keyword: string]: string } = {
    ...
    rainbow: "🌈", wave: "🏄‍♂️", luck: "🍀", beautiful: "😍", crazy: "🤪"
}

function pickEmojiForLyric(text: string): string {
    if (!text) return "🎶";
    for (const key in emojiByLyric) {
        if (text.toLowerCase().includes(key)) return emojiByLyric[key];
    }
    return "🎶";
}
